...
def build_job_key(image_path: Path, scene_name: str, job_version: str) -> str:
    stat = image_path.stat()
    return (
        f"{image_path.resolve()}::size={stat.st_size}::mtime_ns={stat.st_mtime_ns}"
        f"::scene={scene_name}::job_version={job_version}"
    )
...
def save_output_image(output_dir: Path, source_image: Path, scene_name: str, response: dict) -> Path:
    ...
    out_name = f"{slugify(source_image.stem)}__{scene_name}__{slugify(prompt_id)}.png"
...
def process_cycle(
    ...
    for image_path in candidates:
        ...
        for scene in scenes:
            ...
            job_key = build_job_key(image_path, scene_name, args.job_version)
        ...
        if all_scene_ok and args.move_processed_dir is not None:
...
def main() -> int:
    ...
    if args.once:
        total, ok = process_cycle(args, scenes, manifest_path, processed_success, seed_counter)
